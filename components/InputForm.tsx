...
import { 
  ...
  ScanLine, Radio, CheckCircle2, Zap
} from 'lucide-react';
...
const ANALYSIS_STEPS = [
  "Initialisiere Core...",
  "Scanne Makro-Lage...",
  "Validiere Mietspiegel...",
  "Analysiere Faktoren...",
  "Berechne Modernisierung...",
  "Vergleichsobjekte...",
  "Simuliere Potential...",
  "Erstelle Exposé..."
];
...
const InputForm: React.FC<InputFormProps> = ({ isLoading, onSubmit }) => {
  ...
  // Lade-Logik: Beschleunigt (ca. 2-3 Sekunden visuell)
  useEffect(() => {
    let interval: any;
    if (isLoading) {
      setProgress(0);
      interval = setInterval(() => {
        setProgress(prev => {
          // Stopp bei 99%, bis das Ergebnis tatsächlich da ist
          if (prev >= 99) return 99;
          
          // Schnellerer Inkrement: Durchschnittlich 3% pro 50ms
          // Gesamtdauer ca. 1.6 - 2.0 Sekunden bis 99%
          const increment = Math.random() * 2 + 2; 
          return Math.min(prev + increment, 99);
        });
      }, 50); // Schnellerer Tick (50ms)
    } else {
      setProgress(0);
    }
  ...
  // Berechne den aktuellen Text-Schritt basierend auf dem Fortschritt (0-100)
  ...
  useEffect(() => {
    ...
    // Debounce reduziert auf 200ms für schnelleres Feedback
  ...
  const handleSelectSuggestion = (s: any) => {
    ...
    setFormData(prev => ({ ...prev, address: `${p.street || p.name || ''} ${p.housenumber || ''}, ${p.postcode || ''} ${p.city || ''}` }));
      ...
      <div className="relative min-h-[600px]">
          ...
          <div className="absolute inset-0 z-[200] bg-[#020617]/95 backdrop-blur-xl flex flex-col items-center justify-center p-12 text-center animate-fade-in">
             <div className="w-full max-w-md flex flex-col items-center relative">
                ...
                <div className="relative w-64 h-64 mb-12 flex items-center justify-center">
                   {/* Pulsing Rings - Schneller */}
                   <div className="absolute inset-0 border border-[#f5931f]/20 rounded-full animate-[ping_1.5s_linear_infinite]"></div>
                   <div className="absolute inset-4 border border-[#f5931f]/10 rounded-full animate-[ping_1.5s_linear_infinite_0.5s]"></div>
                   <div className="absolute inset-12 border border-white/5 rounded-full"></div>
                   
                   {/* Rotating Scanner - Schneller */}
                   <div className="absolute inset-0 rounded-full animate-[spin_1s_linear_infinite] border-t-2 border-[#f5931f]/50 border-r border-r-transparent border-l border-l-transparent border-b border-b-transparent shadow-[0_-10px_40px_rgba(245,147,31,0.2)]"></div>
                ...
                <div className="space-y-6 w-full relative">
                  <div className="h-10 flex items-center justify-center overflow-hidden">
                    <p key={currentStepIndex} className="text-xs font-black text-white uppercase tracking-[0.15em] animate-in slide-in-from-bottom-1 fade-in duration-200 text-center">
                  ...
                  <div className="w-full h-1 bg-white/5 rounded-full overflow-hidden relative">
                    <div 
                      className="absolute inset-y-0 left-0 bg-[#f5931f] transition-all duration-100 ease-out"
                  ...
                  <div className="flex justify-between text-[9px] text-slate-500 font-bold uppercase tracking-widest px-1">
                    ...
                    <span className="animate-pulse text-[#f5931f]">Live-Analyse</span>
                    <span>Fertig</span>
                  </div>
                </div>
        ...
        <form onSubmit={handleSubmit} className="p-8 lg:p-12 space-y-8">
            ...
            {showSuggestions && suggestions.length > 0 && (
              <ul className="absolute z-[60] left-0 right-0 mt-3 bg-[#1e293b] border border-white/10 rounded-2xl shadow-2xl overflow-hidden ring-1 ring-black/50">
                {suggestions.map((s, idx) => (
                  <li key={idx}>
                    <button type="button" onClick={() => handleSelectSuggestion(s)} className="w-full text-left px-6 py-4 hover:bg-[#f5931f]/10 text-slate-300 text-xs border-b border-white/5 last:border-0 transition-colors">
                      {s.properties.street || s.properties.name} {s.properties.housenumber || ''}, {s.properties.postcode} {s.properties.city}
                    </button>
                  </li>
                ))}
              </ul>
          ...
          <div className="space-y-6 pt-6 border-t border-white/5">
             ...
             {/* Dynamic Warning Indicator for High Floor / No Elevator */}
          ...
          <button type="submit" disabled={isLoading || !formData.address}
            className="w-full py-6 bg-[#f5931f] text-white font-black rounded-2xl shadow-2xl hover:bg-white hover:text-slate-950 transition-all flex items-center justify-center gap-4 text-xs uppercase tracking-[0.3em] group mt-4 overflow-hidden relative"
          >
